...
`src/data/mockData.js` 파일만 수정하면 원하는 영상으로 바로 테스트해볼 수 있습니다.

**필요한 데이터**: 유튜브 영상 ID, 원하는 제목/설명, 영상 길이

1. **영상 ID 확보**: 유튜브 URL `v=` 뒤의 11자리 코드를 복사합니다. (예: `https://www.youtube.com/watch?v=Ke90Tje7VS0` -> `Ke90Tje7VS0`)
2. **파일 수정**: `src/data/mockData.js`를 열고 `mockSeries` 객체를 수정합니다.
   - **시리즈 정보**: `title`, `creator`, `description`을 내 시리즈에 맞게 변경하세요.
   - **영상 목록**: `videos` 배열 안의 `id`를 위에서 복사한 ID로 교체합니다.
3. **확인**: 저장 후 브라우저를 새로고침하면 내 영상으로 바뀐 것을 확인할 수 있습니다. 
   - *팁: 썸네일은 ID만 바꾸면 자동으로 적용됩니다!*
